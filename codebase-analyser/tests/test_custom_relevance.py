...
def main():
    ...
    try:
        ...
        # Since we're having issues with the LanceDB search API, let's test the combined scoring directly
        ...
        for chunk in all_chunks:
            ...
            chunk["graph_proximity"] = 1.0 / (1.0 + graph_distance)
            print(f"Chunk {chunk['node_id']}: Graph Distance = {graph_distance}, Proximity = {chunk['graph_proximity']:.4f}")

        # Calculate combined scores
        print("\nCalculating combined scores (alpha={}, beta={})...".format(args.alpha, args.beta))
        for chunk in all_chunks:
            chunk["combined_score"] = args.alpha * chunk["semantic_score"] + args.beta * chunk["graph_proximity"]
