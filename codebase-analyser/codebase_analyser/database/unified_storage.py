...
class UnifiedStorage:
    ...
    def _build_graph_from_dependencies(self) -> nx.DiGraph:
        ...
        dependencies = self.db_manager.get_dependencies()
        ...
        for dep in dependencies:
            ...
            G.add_edge(source_id, target_id, type=dep_type, weight=strength)

        return G

    def _calculate_graph_distance(self, G: nx.DiGraph, source_id: str, target_id: str) -> float:
        ...
        try:
            ...
            path_length = nx.shortest_path_length(G, source=source_id, target=target_id)
            return float(path_length)
        except nx.NetworkXNoPath:
            try:
                # Try reverse direction
                path_length = nx.shortest_path_length(G, source=target_id, target=source_id)
                return float(path_length)
            except nx.NetworkXNoPath:
                try:
                    # Try undirected path
                    UG = G.to_undirected()
                    path_length = nx.shortest_path_length(UG, source=source_id, target=target_id)
                    return float(path_length)
                except nx.NetworkXNoPath:
                    # No path exists
                    return float('inf')
    ...
    def search_with_combined_scoring(
        ...
        Args:
            query_embedding: Query embedding vector
            query_node_id: ID of the query node for graph distance calculation (optional)
        ...
        Returns:
            List of matching code chunks with combined scores
        """
        # Validate weights
        if alpha < 0.0 or alpha > 1.0 or beta < 0.0 or beta > 1.0:
            raise ValueError("Weights alpha and beta must be between 0.0 and 1.0")

        if abs(alpha + beta - 1.0) > 1e-6:
            logger.warning(f"Weights alpha ({alpha}) and beta ({beta}) do not sum to 1.0. Normalizing.")
            total = alpha + beta
            alpha = alpha / total
            beta = beta / total

        # First, search by vector similarity to get candidates
        results = self.db_manager.search_code_chunks(
            query_embedding=query_embedding,
            limit=limit * 3  # Get more results than needed for reranking
        )

        # If no query node ID is provided, return results based on semantic similarity only
        if not query_node_id or not results:
            return results[:limit]

        # Build the graph from dependencies
        G = self._build_graph_from_dependencies()

        # If the query node is not in the graph, return results based on semantic similarity only
        if not G.has_node(query_node_id):
            return results[:limit]

        # Calculate combined scores
        scored_results = []
        for result in results:
            # Get semantic similarity score (already normalized between 0 and 1)
            semantic_score = result.get("_distance", 0.0)
            if semantic_score > 1.0:
                semantic_score = 1.0

            # Calculate graph distance
            graph_distance = self._calculate_graph_distance(G, query_node_id, result["node_id"])
            ...
            result["graph_proximity"] = graph_proximity
