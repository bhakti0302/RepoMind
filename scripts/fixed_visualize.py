#!/usr/bin/env python3
"""
Visualize code relationships from the database with fixed JSON output.

This script reads the code chunks from the database and visualizes the relationships,
ensuring that the JSON output is properly separated from log messages.
"""
...
# Import from the codebase_analyser package
try:
    from codebase_analyser.database.unified_storage import UnifiedStorage
except ImportError:
    logger.error("Could not import UnifiedStorage. Make sure the codebase_analyser package is installed.")
    sys.exit(1)
...
RELATIONSHIP_COLORS = {
    'CONTAINS': '#1f77b4',  # Blue
    'IMPORTS': '#ff7f0e',  # Orange
    'EXTENDS': '#2ca02c',  # Green
    'IMPLEMENTS': '#d62728',  # Red
    'CALLS': '#9467bd',  # Purple
    'USES': '#8c564b',  # Brown
    'REFERENCES': '#e377c2',  # Pink
    'DEFAULT': '#7f7f7f'  # Gray
}
...
def main():
    ...
    parser.add_argument("--timestamp", help="Timestamp to include in the filename")
    ...
    try:
        ...
        # Print ONLY the JSON output to stdout, with a marker to help parsing
        result = {
            "multi_file_relationships": multi_file_path,
            "relationship_types": relationship_types_path
        }
        
        # Print a special marker followed by the JSON
        print("JSON_OUTPUT_BEGINS")
        print(json.dumps(result))
        print("JSON_OUTPUT_ENDS")

    except Exception as e:
        logger.error(f"Error visualizing code relationships: {e}")
